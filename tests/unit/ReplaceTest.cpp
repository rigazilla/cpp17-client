 ...
 * REPLACE Unit Tests
 *
 * Request format (opcode 0x07) - identical layout to PUT:
...
TEST(ReplaceTest, EncodeDefaultExpiration) {
    ByteArray request;
    ...
    EXPECT_EQ(0x07, request[3]);  // opcode
...
// Test 2: maxIdle set (low nibble = SECONDS 0x0), lifespan default (high 0x7)
TEST(ReplaceTest, EncodeMaxIdleOnly) {
    ByteArray request;
    HeaderCodec::writeRequestHeader(request, makeHeader());
    ...
    EXPECT_EQ(45u, Codec::readVLong(request, off));
    EXPECT_EQ(value, Codec::readByteArray(request, off));
}

// Test 3: both lifespan and maxIdle set -> two vLongs, lifespan first
TEST(ReplaceTest, EncodeBothExpirations) {
    ByteArray request;
    HeaderCodec::writeRequestHeader(request, makeHeader());
...
TEST(ReplaceTest, OpcodeConstants) {
    EXPECT_EQ(0x07, Opcode::REPLACE_REQUEST);
    EXPECT_EQ(0x08, Opcode::REPLACE_RESPONSE);
}
