...
TEST_F(HashAwareRoutingIntegrationTest, FailoverToBackupOwner)
{
   ...
   ASSERT_TRUE(found.has_value()) << "Key should exist after PUT";
   EXPECT_EQ(value, found.value()) << "Retrieved value should match";
   ...
   std::optional<ByteArray> foundAfterFailover;
   ...
   for (int attempt = 0; attempt < 3; attempt++)
   {
      try
      {
         foundAfterFailover = cache->get(key).get();
         if (foundAfterFailover.has_value())
         {
            break;
         }
      }
   ...
   EXPECT_EQ(value, foundAfterFailover.value())
...
TEST_F(HashAwareRoutingIntegrationTest, TopologyRebalanceUpdatesRouting)
{
   ...
   for (const auto &keyStr : testKeys)
   {
      ...
      for (int attempt = 0; attempt < 3; attempt++)
      {
         try
         {
            found = cache->get(key).get().has_value();
            if (found)
               break;
         }
         catch (...)
